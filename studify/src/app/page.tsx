...
function MiniGPARing({ gpa }: { gpa: number }) {
  const r = 22;
  ...
  return (
    <div className="relative w-14 h-14 shrink-0 flex items-center justify-center">
      <svg className="absolute w-full h-full -rotate-90" viewBox="0 0 56 56">
        <circle cx="28" cy="28" r={r} stroke="#e5e7eb" strokeWidth="5" fill="transparent" className="dark:stroke-[#3F3F46]" />
        <motion.circle
          cx="28" cy="28" r={r}
          stroke={color}
          strokeWidth="5"
      ...
      <span className="text-[11px] font-black z-10" style={{ color }}>{gpa.toFixed(1)}</span>
    </div>
  );
}
...
export default function Home() {
  ...
  const [attendance, setAttendance] = useState({
    skipsLeft: 0, isSafe: true,
    worstSubject: "", worstPct: 0, totalSubjects: 0,
  });
  ...
  useEffect(() => {
    const unsubscribe = onAuthStateChanged(auth, async (currentUser) => {
      ...
      // Attendance — find the most-at-risk subject
      ...
      if (attSnap.exists() && attSnap.data().list?.length > 0) {
        ...
        setAttendance({
          skipsLeft: minSkips,
          isSafe: enriched.every((x) => x.safe),
          worstSubject: worst.name,
          worstPct: worst.pct,
          totalSubjects: list.length,
        });
      }

      // Exam — soonest upcoming
      ...
      if (examSnap.exists() && examSnap.data().list?.length > 0) {
        ...
        const withDays = list.map((e) => {
          ...
          return { ...e, daysLeft: Math.ceil(diff / (1000 * 3600 * 24)) };
        });
      ...
      // GPA
      const gradesSnap = await getDoc(doc(db, "users", currentUser.uid, "grades", "data"));
      if (gradesSnap.exists() && gradesSnap.data().list?.length > 0) {
        const list: any[] = gradesSnap.data().list;
        const totalCredits = list.reduce((a, s) => a + s.credits, 0);
        if (totalCredits > 0) {
          const weighted = list.reduce((a, s) => a + getGradePoints(s.percentage) * s.credits, 0);
          setGpaData({ gpa: Math.round((weighted / totalCredits) * 100) / 100, hasData: true });
        }
      }
  ...
  return (
    <div className="flex min-h-screen flex-col bg-[#FDFBF7] dark:bg-[#1C1917] text-[#2D3436] dark:text-[#E7E5E4] px-6 safe-top pb-6 font-sans transition-colors duration-500 md:px-10 md:py-8">
      ...
      >
        {/* Left: name + subtitle row — allow shrink so it never overflows */}
        ...
        {/* Right: icon buttons (mobile only) — never shrink */}
      ...
      <div className="md:max-w-5xl md:mx-auto md:w-full md:grid md:grid-cols-[1.15fr_1fr] md:gap-6 md:items-start">
        ...
        >
          <div className="absolute top-4 left-0 w-full h-full bg-[#E8E8E4] dark:bg-black/40 rounded-[30px] shadow-sm transform scale-95 opacity-60 z-0 transition-colors" />
        ...
        <div className="flex flex-col gap-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <Link href="/exam">
              <motion.div
                ...
                whileTap={{ scale: 0.97 }}
                className={`rounded-[25px] p-5 h-48 flex flex-col justify-between shadow-lg cursor-pointer hover:scale-[1.03] transition-transform duration-200 ${getExamTheme()}`}
              >
                ...
                <div className="space-y-1">
                  ...
                  <div className="w-full bg-black/10 dark:bg-black/40 h-1.5 rounded-full overflow-hidden">
                    <div className="h-full bg-white/90 dark:bg-current transition-all duration-1000 rounded-full" style={{ width: `${examData.prepLevel}%` }} />
                  </div>
                </div>
              </motion.div>
            </Link>
            ...
            <Link href="/attendance">
              <motion.div
                ...
                whileTap={{ scale: 0.97 }}
                className="bg-[#FFDAC1] text-[#B07D62] dark:bg-[#292524] dark:text-orange-200 dark:border-2 dark:border-orange-400/20 rounded-[25px] p-5 h-48 flex flex-col justify-between shadow-lg cursor-pointer hover:scale-[1.03] transition-transform duration-200"
              >
                <div className="flex justify-between items-center opacity-70">
          ...
          <Link href="/grades">
            <motion.div
              ...
              whileTap={{ scale: 0.98 }}
              className="w-full bg-white dark:bg-[#292524] dark:border dark:border-[#44403C] rounded-[25px] p-5 flex items-center gap-4 shadow-sm cursor-pointer hover:scale-[1.02] transition-transform duration-200"
            >
              {gpaData.hasData ? (
                <MiniGPARing gpa={gpaData.gpa} />
              ) : (
                <div className="w-14 h-14 rounded-full bg-gray-100 dark:bg-[#3F3F46] flex items-center justify-center shrink-0">
                  <GraduationCap size={22} className="text-gray-300 dark:text-gray-600" />
                </div>
              )}
              <div className="flex-1 min-w-0">
                <p className="text-[10px] font-bold uppercase tracking-widest text-gray-400 mb-0.5">GPA Tracker</p>
                {gpaData.hasData ? (
                  <>
                    <h3 className="text-2xl font-black text-[#2D3436] dark:text-white">{gpaData.gpa.toFixed(2)}</h3>
                    <p className="text-xs text-gray-400 font-medium">out of 4.00</p>
                  </>
                ) : (
                  <>
                    <h3 className="text-base font-bold text-[#2D3436] dark:text-white">Track your GPA</h3>
                    <p className="text-xs text-gray-400">Add subjects to get started</p>
                  </>
                )}
              </div>
          ...
          <Link href="/simulator">
            <motion.div
              ...
              whileTap={{ scale: 0.98 }}
              className="w-full bg-[#2D3436] dark:bg-[#292524] dark:border dark:border-[#44403C] text-white dark:text-[#E7E5E4] p-5 rounded-[25px] flex items-center justify-between shadow-xl cursor-pointer hover:scale-[1.02] transition-transform duration-200"
            >
